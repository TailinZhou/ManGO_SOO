...
class DatasetBuilder(abc.ABC):
    ...
    def __init__(self, x_shards, y_shards, internal_batch_size=32,
         ...
         #normalization_type: zero_unit_variance for superconduct, min_max for other tasks, due to some unknown bugs in superconduct with randon forestry
        ...
        """
        self.normalization_type =  normalization_type or os.getenv(
        ...
        for x_batch  in self.iterate_batches(self.internal_batch_size, return_y=False):
            if sample_num == 0:
                self.x_min = np.full((1, x_batch.shape[1]), np.inf, dtype=self.input_dtype)
                self.x_max = np.full((1, x_batch.shape[1]), -np.inf, dtype=self.input_dtype)
            batch_size = x_batch.shape[0]
            # Update min/max
            self.x_min = np.minimum(self.x_min, np.min(x_batch, axis=0, keepdims=True))
            self.x_max = np.maximum(self.x_max, np.max(x_batch, axis=0, keepdims=True))
        ...
        for i, y in enumerate(self.iterate_samples(return_x=False)):
            ...
            self.y_min = np.minimum(self.y_min, np.min(y))
            self.y_max = np.maximum(self.y_max, np.max(y))
    ...
    def normalize_y(self, y):
        ...
        """
        if self.normalization_type == "min_max":
            y_return = (y - self.y_min) / (self.y_max - self.y_min  + 1e-8 )
            if np.isscalar(y_return):
    ...
    def denormalize_y(self, y):
        ...
        """
        if self.normalization_type == "min_max":
            y_return = y * (self.y_max - self.y_min) + self.y_min
            if np.isscalar(y_return):
                y_return = np.array([[y_return]])
            return y_return
        else:
