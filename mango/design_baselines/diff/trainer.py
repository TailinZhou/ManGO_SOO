...
def split_dataset(task, val_frac=None, device=None, temp=None,augment=False):
    ...
    print('sample x (check normalization or not) \n', x[:5])
    print('sample y (check normalization or not) \n', y[:5])
    print('sample w (check normalization or not) \n', w[:5])
    # TODO: Modify
    # full_ds = DKittyMorphologyDataset()
    # y = (y - full_ds.y.min()) / (full_ds.y.max() - full_ds.y.min())
...
class RvSDataModule(pl.LightningDataModule):
    ...
    def __init__(self, task, batch_size, num_workers, val_frac, device, temp, augment=False):
        super().__init__()
    ...
    def setup(self, stage=None):
        self.train_dataset, self.val_dataset = split_dataset(
            self.task, self.val_frac, self.device, self.temp, self.augment)
    ...
    def train_dataloader(self):
        # g = torch.Generator()   
        # g.manual_seed(int(os.environ.get("SEED", "0")))   
        # 创建 WeightedRandomSampler
        # hist, bin_edges = torch.histogram(torch.tensor(y.sum(axis=1)), bins=25)
        # weights = get_weights_v2(self.train_dataset.y.sum(axis=1), temp=self.temp)
